#' Simulate periodic-global changepoint series (if no global changes, then a time series with stable periodicity is generated)
#'
#' @param period CHANGEME
#' @param dist CHANGEME
#' @param glob.cpts CHANGEME
#' @param per.cpts CHANGEME
#' @param params CHANGEME
#' @param reps CHANGEME
...
circcptSim.bern=function(period,length,per.cpts,params,reps){
  ...
  onedataset=function(period,length,per.cpts,params){
    ...
    data=rbinom(length,size=1,prob=(means.per/10))
    return(data)
  }
