setClass("cpt",slots=list(data.set="ts", cpttype="character", test.stat="character",pen.type="character",pen.value="numeric",minseglen="numeric",cpts="numeric",cpts.global="numeric",cpts.periodic="numeric",ncpts.max="numeric",param.est="list",date="character",version="character"),prototype=prototype(cpttype="Not Set",date=date(),version=as(packageVersion("changepoint.periodic"),'character')))

setClass("cpt.reg",slots=list(data.set="matrix", cpttype="character", test.stat="character",pen.type="character",pen.value="numeric",minseglen="numeric",cpts="numeric",cpts.global="numeric",cpts.periodic="numeric",ncpts.max="numeric",param.est="list",date="character",version="character"),prototype=prototype(cpttype="regression",date=date(),version=as(packageVersion("changepoint.periodic"),"character")))
...
#setMethod("cpts.global","cpt",function(object) object@cpts.global[-length(object@cpts.global)])
#setMethod("cpts.global","cpt.reg",function(object) object@cpts.global[-length(object@cpts.global)])
#setMethod("cpts.periodic","cpt",function(object) object@cpts.periodic[-length(object@cpts.periodic)])
#setMethod("cpts.periodic","cpt.reg",function(object) object@cpts.periodic[-length(object@cpts.periodic)])
...
# plot functions
setMethod("plot","cpt",function(x,cpt.col='red',cpt.width=1,cpt.style=1,cpts.periodic=TRUE,cpts.global=TRUE,...){
  ...
  plot(data.set.ts(x),...)
  cpts = list()
  # TODO: Color periodic and global changepoints
  if (cpts.periodic && cpts.global) {
    cpts=c(0,cpts(x))
  }
  else if (cpts.periodic) {
    cpts=c(0,cpts.periodic(x))
  }
  else if (cpts.global) {
    cpts=c(0,cpts.global(x))
  }
  else {
    return()
  }

  if(cpttype(x)=="Normal mean"  ||  cpttype(x)=="Normal meanvar"){
